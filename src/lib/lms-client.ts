...
export class LmsClient {
    private env: LmsEnvironment;
    private token: string | null = null;
    private baseParams: Record<string, any> = {};
    private requestHistory: LmsRequestHistory[] = [];
    ...
    async login(): Promise<LmsLoginResult> {
        ...
        const payload = {
            lname: this.env.user_code,
            pass: this.env.pass_master,
        };

        console.log("[LmsClient] Logging in...", {
            host: this.env.host,
            user: this.env.user_code,
        });
    ...
    async send(options: LmsSendOptions): Promise<LmsSendResult> {
        const { path, payload = {}, method = "POST", dmn } = options;
        ...
        try {
            ...
            if (data.encrypted_data) {
                try {
                    ...
                    console.warn("[LmsClient] Failed to decrypt response:", e);
                }
            }
